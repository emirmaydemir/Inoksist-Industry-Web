...
import navData2 from "@/locales/en/navLinks.json";
...
const Header2 = () => {
  ...
  const navLinks = navData2.navLinks;
  const search = navData2.search;
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        module = await import("@/locales/en/allData");
  ...
  useEffect(() => {
    const currentPath = language === "en" && pathname === "/en" ? "/" : pathname.replace(/^\/en/, ""); // Geçerli rotayı al en değerini çıkartıyoruz hatalı rota tespiti yapmamak için
